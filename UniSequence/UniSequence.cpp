...
{
	log("=======End log function was called, logging will now stop and plugin unloaded.=======");
...
void UniSequence::SyncSeq(string callsign, int status)
{
	...
	{
		string err = e.what();
		log("plugin exception detected: " + err);
	}
}
...
void UniSequence::SyncSeqNum(string callsign, int seqNum)
{
	try
	{
		...
		for (auto& seqN : sequence)
		{
			...
			{
	#ifdef ENV_DEBUG
				OutputDebugStringA("Synchronizing sequence number of " + *callsign.c_str());
	#endif // ENV_DEBUG
		...
		while (true)
		{
			for (auto& airport : airportList)
			{
				...
				if (auto result = requestClient.Get(SERVER_RESTFUL_VER + airport + "/queue"))
				{
					...
					{
						log("Updating data for " + seqObj["callsign"]);
...
SeqN* UniSequence::GetSeqN(CFlightPlan fp)
{
	try
	{
		...
		log("Getting Sequence Node of " + cs);
...
void UniSequence::RemoveFromSeq(CFlightPlan fp)
{
	...
	{
		string cs = fp.GetCallsign();
		log("Removing " + cs + "from local sequence");
		int offset = 0;
		for (auto& ac : sequence)
		{
			if (ac.fp.GetCallsign() == fp.GetCallsign())
			{
				...
				offsetStr[3] = '\0';
				log(cs + " was found with offset of " + offsetStr);
				log("Removing");
				sequence.erase(sequence.begin() + offset);
				log(cs + " has been removed");
				return;
			}
			offset++;
		}
		log(cs + " is not in local sequence, nothing happend.");
	}
	catch (const std::exception& e)
	{
		string err = e.what();
...
void UniSequence::CheckApEnabled(string depAirport)
{
	try
	{
		for (auto& airport : airportList)
		{
			...
			{
				log("Airport " + depAirport + " is in the list, skipped.");
				return;
			}
		}
